...
| **2 · the persisting deck** | **DONE and live, 2026-09-01, version 0.1.64** — `e1e9ca7`. `baceb0d` had cut sentence three, the provenance claim; this cut one word from sentence two. *"Below"* is deictic — it points at whatever pane is on, and was false on eight of thirteen. Without it the sentence is a claim about the four challenges, whose antecedent is the link immediately before it, and it holds from anywhere. Deleting the sentence was the other option and the worse one: it is the only place the page says what kind of evidence it holds. |
| **3 · nav bar / pipeline** | **DONE and live, 2026-09-01, version 0.1.65** — `41b4461`, rebuilt `e507365`. **Not a nav bar**, and the reason is in the commit: the rail is already the nav and already reflows into a horizontal strip below 860px, so a top bar would be the second nav on a laptop and the third on a phone; and the four groups are not a sequence, so a pipeline would draw a progression the page does not have. **What the strip is actually for** is the reading depth control, which lived in the non-sticky masthead and scrolled away on every pane switch — the page had two controls and only one followed you. On the twelve non-overview views the deck now collapses to `IT LOOKED RIGHT · group · pane` and the bar sticks with the control in it; overview is untouched. ⚠️ **Three things the sticky bar breaks that only a browser finds** — the sticky rail slid under it (now derives from `--mast-h`), `show()`'s scroll landed the pane heading beneath it, and **the browser's own `#hash` scroll runs after ours and ignores it**, so `scroll-margin-top` was required as well. |
