...
| **M11 · A guard can now arrive in a new project instead of being remembered into it** | N3's first decision — *does a gate get pushed, or pulled?* — answered by building the pulled version. `starter/` is the mechanical half of the runbook prebuilt, on one rule: **prebuild what has a single right answer, leave blank what contains a decision**, and nothing appears on both sides. It does not claim its own success — `starter/bootstrap.sh` ends by handing to `starter/tools/check_setup.sh`, a separate program that goes and looks — and it proves the commit hook **fires** by building a throwaway repository, committing into it and reading the message back, then requires that check to go red under four mutations. **What has not happened is a walk from a clean machine**, which the handout says on its own face | `built` | `a6eb8f0` | `starter/README.md` | `current` |
