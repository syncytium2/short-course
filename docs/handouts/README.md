...
> **⚠ Two sheets here now serve two different jobs, added 2026-09-08.** The winter workshop made
> `before-the-day.html` the thing a participant is actually handed, and **demoted Cold Start to the
> back pocket** — the reference you open when forty minutes was not enough. Cold Start is unchanged
> and nothing in it was deleted; what changed is who reads it and when. The workshop's own record is
> [`../workshop/HANDOFF.md`](../workshop/HANDOFF.md).

| File | What it is | Live at |
|---|---|---|
| [`before-the-day.html`](before-the-day.html) | **Before the Day** — the workshop's pre-session sheet, and **the shortest thing in this folder on purpose**. Four conditions, each written as something the reader can *prove* rather than a step they can follow: an agent that returns a file when asked for one; an agent that can say something about a folder the reader did not type into it; a real problem in one paragraph; and the data for it, opening **with the network off**. The second condition is the one that earns its place — reading your files and talking about your files produce identical-looking replies, and one minute of asking for something the agent could only know by looking is the only thing that separates them. **Not in [`../../tools/pages.txt`](../../tools/pages.txt) and not published**, because the workshop material stays off the public site while the workshop is still a proposal. It inherits `terms_check` anyway, which globs this folder. **Never murderboarded.** | **new 2026-09-08** · [artifact](https://claude.ai/code/artifact/1085b4d4-7669-4711-91fa-b1204ce3b73f) (private) |
| [`the-short-path.html`](the-short-path.html) | **The Short Path** — four steps from nothing to a working web page, on Tony's outline of 2026-09-14: pay for Claude Code, open the Claude app, paste one prompt that has the app guide the VS Code and extension install, then tell Claude Code the goal. **It can be this short because it writes no install instructions**: the app gives them live and reads screenshots, which is what `show-it-your-screen.html` found works where advance click paths do not. It carries only what the app cannot be relied on to know — which plan (Free does not include Claude Code, read on claude.com 2026-09-14), the two prompts, Cold Start's workspace-trust trap, and how to check the result with an input whose answer you already know. Links Cold Start as the long version. **Not in [`../../tools/pages.txt`](../../tools/pages.txt) and not deployed**; whether it goes public is his call. **Never walked, never murderboarded.** | **new 2026-09-14** · [artifact](https://claude.ai/artifact/MRgeK3sMfABJwUowr1X6tW) (private) |
| [`cold-start.html`](cold-start.html) | **Cold Start** — the setup runbook that comes before the other two. **One document, three routes**, picked at the top and *named* there — **the short route**, **the laptop route**, **the cluster route**. The short route was **added 2026-09-14** as the first button: four steps of its own, S1–S4, folded in from [`the-short-path.html`](the-short-path.html), and **nothing keeps the two in step**. A previous third, the browser route, was **removed 2026-09-09** and is parked at [`../parked/browser-route.html`](../parked/browser-route.html): its only walk by a beginner failed, every hard step on it happened in a vendor web UI the agent could not see, and it could not satisfy `before-the-day.html`'s requirement of an agent that can reach your own files. They are named rather than described because the prose refers back to them, and describing them produced *"you picked the browser at the top of this page"*, which to a beginner means the thing they are reading the page in. It used to say *"34 steps in seven phases"* here; 34 is one route's count, the page holds 39 steps, and no reader is shown all of them — so the count now lives in `tools/tier_check.sh`, which prints all three and is the only place they cannot go stale. The phases: accounts (with the tier — Claude Code, Codex, Gemini CLI, Copilot CLI, Cursor — and why the GitHub account is not optional), storage, machine (including **3.5, putting the agent inside the editor and finding a button that is invisible until a file is open** — and the workspace-trust answer in 3.1 that silently disables the whole extension), the two prompts to paste, reading your own usage, the habits, and claiming a domain (Porkbun → Cloudflare → GitHub Pages). **Every step is a checklist**, red **INCOMPLETE** until every box under it is ticked — the step's state is derived from its own list, so the badge cannot disagree with it. Heading and outbound links stay visible; the prose folds behind them. The three closing traps each carry a real incident linking to **lookedright.tonydefazio.com**. Built for a narrow window docked beside a terminal; ticks persist in the browser. | [artifact](https://claude.ai/code/artifact/d6b80c4f-3d5d-4184-b555-090c7fd7dcbd) (private) |
